...
function boton(){
    var elementBtn = document.createElement('div');
    elementBtn.classList.add('btn');
    elementBtn.innerHTML = 'Carta de tiktok';
    elementBtn.setAttribute('onclick', 'cartaCreada()');
    app.appendChild(elementBtn);
}

boton();

function cartaCreada() {
    if (index <= tiktokers.length -1 ) {
        var elementCard = document.createElement('div');
        var app = document.getElementById('app');
        elementCard.classList.add('card');
        app.appendChild(elementCard);
        imageCreate();
        index++;    
